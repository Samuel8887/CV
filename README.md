...
2. Open the repository folder containing `index.html`.
...
The updated website files live at the repository root. Work on `update-visual`
and merge into `main` when ready to publish. In Settings > Pages, select GitHub
Actions as the source. The workflow publishes pushes to `main` and can also be
run manually. It packages only the public HTML, CSS, JavaScript, favicon, and PDF.
